    ...
    defineLevels() {
        return [
...

            {
                ...
                cars: [
                    // Ruch okrężny po rondzie (promień 230 = średnia R_OUTER i R_ISLAND), kierunek przeciwny do ruchu wskazówek zegara
                    new NpcCar({x: canvas.width/2 + 230, y: canvas.height/2,       speed: 2, type: 'sedan',   color: '#e74c3c', movement: new CircularMovement(canvas.width/2, canvas.height/2, 230, -0.004)}),
                    new NpcCar({x: canvas.width/2 - 115, y: canvas.height/2 + 199, speed: 2, type: 'compact', color: '#9b59b6', movement: new CircularMovement(canvas.width/2, canvas.height/2, 230, -0.004)}),
                    new NpcCar({x: canvas.width/2 - 115, y: canvas.height/2 - 199, speed: 2, type: 'suv',     color: '#1abc9c', movement: new CircularMovement(canvas.width/2, canvas.height/2, 230, -0.004)}),
                ],
                parkingZones: [
                    new ParkingZone({x: canvas.width - 130, y: canvas.height/2 + 35, w: 60, l: 120, angle: 0})
                ],
                curbs: [
                    // Ramię górne (krawężniki na zewnątrz jezdni: ±80 = krawędź jezdni ±70 + pół szerokości curba 10)
                    new Curb(canvas.width/2 - 80, (canvas.height/2 - 280)/2,                            20, canvas.height/2 - 280, Math.PI/2),
                    new Curb(canvas.width/2 + 80, (canvas.height/2 - 280)/2,                            20, canvas.height/2 - 280, Math.PI/2),
                    // Ramię dolne
                    new Curb(canvas.width/2 - 80, canvas.height/2 + 280 + (canvas.height/2 - 280)/2,   20, canvas.height/2 - 280, Math.PI/2),
                    new Curb(canvas.width/2 + 80, canvas.height/2 + 280 + (canvas.height/2 - 280)/2,   20, canvas.height/2 - 280, Math.PI/2),
                    // Ramię lewe
                    new Curb((canvas.width/2 - 280)/2,                          canvas.height/2 - 80,   20, canvas.width/2 - 280,  0),
                    new Curb((canvas.width/2 - 280)/2,                          canvas.height/2 + 80,   20, canvas.width/2 - 280,  0),
                    // Ramię prawe
                    new Curb(canvas.width/2 + 280 + (canvas.width/2 - 280)/2,  canvas.height/2 - 80,   20, canvas.width/2 - 280,  0),
                    new Curb(canvas.width/2 + 280 + (canvas.width/2 - 280)/2,  canvas.height/2 + 80,   20, canvas.width/2 - 280,  0),
